...
Bootstrapping is a useful technique to create ground-truth samples for a null-distribution. However, it is rather costly because the statistic has to be re-computed for every sample. SHOGUN has a bootstrapping implementation, \texttt{bootstrap\_null()}, for any two-sample test in \shogunclass{CTwoSampleTestStatistic}, and for any independence test in \shogunclass{CIndependenceTestStatistic}.
